...
$(call unstyledeverything,$(LATEX),tex): $(FILEPREFIX)$(LATEX)/%.tex: $$(call srcs,$$*) $(YAML) $(srcdir)/default.yml $(srcdir)/template.tex $(srcdir)/pandoc-custom.py $(srcdir)/pandoc-latex.py
	$(makefolders)
	(cat $<; echo; echo; echo "---"; cat $(YAML); $(fileyaml) echo "..."; echo "---"; cat "$(srcdir)/default.yml"; echo "...") | pandoc -f markdown-smart -t latex-smart --standalone --template "$(srcdir)/template.tex" --filter "$(srcdir)/pandoc-custom.py" --filter "$(srcdir)/pandoc-latex.py" -o $@ --top-level-division=chapter $(if $(BIBLIOGRAPHY),--biblatex,)
...
$(FILEPREFIX)$(LATEX)/$(INDEX).tex: $$(call unstyledeverything,$(LATEX),tex) $(YAML) $(srcdir)/default.yml $(srcdir)/pandoc-custom.py $(srcdir)/pandoc-latex.py
	$(makefolders)
	(echo "---"; cat $(YAML); echo "type: index"; echo "..."; echo "---"; cat "$(srcdir)/default.yml"; echo "...") | pandoc -f markdown-smart -t latex-smart --standalone --template "$(srcdir)/template.tex" --filter "$(srcdir)/pandoc-custom.py" --filter "$(srcdir)/pandoc-latex.py" -o $@ --top-level-division=chapter $(if $(BIBLIOGRAPHY),--biblatex,)
...
$(call allfiles,$(HTML),css,xhtml,$(INDEX)): $(FILEPREFIX)$(HTML)/%/$(INDEX).xhtml: $(YAML) $(srcdir)/default.yml $(STYLES)/%.css $(srcdir)/template.xhtml $(srcdir)/pandoc-custom.py $$(wildcard $(STYLES)/%.py) $(srcdir)/pandoc-html.py
	$(makefolders)
	(echo '```{=html}'; echo '<nav xmlns:epub="http://www.idpf.org/2007/ops" epub:type="toc" role="doc-toc">'; echo '```'; echo; echo '$(octothorpe) []{data-from-metadata="localization-type-index"}'; echo;$(foreach standalone,$(allstandalonenames), echo '$(octothorpe). [$(standalone)]($(call relativepath,$(INDEX).xhtml,$(standalone)).xhtml$(octothorpe)BookGen.main)';) echo '$(octothorpe). [[]{data-from-metadata="localization-type-index"}]($(call relativepath,$(INDEX).xhtml,$(INDEX)).xhtml$(octothorpe)BookGen.main)';$(if $(allchapternames), echo '$(octothorpe).'; echo '::: plain :::'; echo '[]{data-from-metadata="localization-chapters"}'; echo ':::::::::::::';$(foreach chapter,$(allchapternames), echo '    $(octothorpe). [[]{data-from-metadata="localization-type-chapter"} $(chapter)]($(call relativepath,$(INDEX).xhtml,$(CHAPTERPREFIX)$(chapter)).xhtml$(octothorpe)BookGen.main)';))$(if $(allappendixnames), echo '$(octothorpe).'; echo '::: plain :::'; echo '[]{data-from-metadata="localization-appendices"}'; echo ':::::::::::::';$(foreach appendix,$(allappendixnames), echo '    $(octothorpe). [[]{data-from-metadata="localization-type-appendix"} $(appendix)]($(call relativepath,$(INDEX).xhtml,$(APPENDIXPREFIX)$(appendix)).xhtml$(octothorpe)BookGen.main)';))$(if $(BIBLIOGRAPHY), echo '$(octothorpe). [[]{data-from-metadata="localization-type-biblio"}]($(call relativepath,$(INDEX).xhtml,$(basename $(BIBLIOGRAPHY))).xhtml$(octothorpe)BookGen.main)';) echo; echo '```{=html}'; echo '</nav>'; echo '```'; echo "---"; echo "style: $(call styles,$@)"; echo "styles:"; echo "- name: $(call styles,$@)"; echo "  css: |"; echo '    ```{=html}'; cat $(STYLES)/$*.css | sed 's/^/    /'; echo '    ```';$(if $(ALLSTYLES),$(foreach style,$(filter-out $*,$(stylenames)), echo "- name: $(style)"; echo "  css: |"; echo '    ```{=html}'; cat $(STYLES)/$*.css | sed 's/^/    /'; echo '    ```';)) cat $(YAML); echo "type: index"; echo "self: $(call relativepath,$(INDEX).xhtml,$(INDEX)).xhtml"; echo "index: $(call relativepath,$(INDEX).xhtml,$(INDEX)).xhtml";$(if $(BIBLIOGRAPHY), echo "biblio: $(call relativepath,$(INDEX).xhtml,$(basename $(BIBLIOGRAPHY))).xhtml";) echo "..."; echo "---"; cat "$(srcdir)/default.yml"; echo "...") | pandoc -f markdown-smart -t html5-smart --standalone --template "$(srcdir)/template.xhtml" --filter "$(srcdir)/pandoc-custom.py"$(if $(wildcard $(STYLES)/$*.py), --filter $(STYLES)/$*.py) --filter "$(srcdir)/pandoc-html.py" $(if $(BIBLIOGRAPHY),--filter pandoc-citeproc) -o $@ --self-contained
...
$(patsubst $(FILEPREFIX)%,%,$(call allfiles,$(BUILD),cls,xxx,GO)): $(BUILD)/%/GO.xxx: $(YAML) $(srcdir)/default.yml $(srcdir)/template-index.tex $(srcdir)/pandoc-custom.py $(wildcard $(STYLES)/%.py)
	$(makefolders)
	(echo "---"; cat $(YAML); echo "style: $*"; echo "..."; echo "---"; cat "$(srcdir)/default.yml"; echo "...") | pandoc -f markdown -t latex --standalone --template "$(srcdir)/template-index.tex" --filter "$(srcdir)/pandoc-custom.py"$(if $(wildcard $(STYLES)/$*.py), --filter $(STYLES)/$*.py,) -o $@
