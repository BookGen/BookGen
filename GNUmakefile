...
localizationyaml = echo "---"; echo "noun: $(call localizedtypes,$<)"; echo "localization:";$(foreach lstr,APPENDICES:appendices APPENDIX:appendix BIBLIO:biblio CHAPTER:chapter CHAPTERS:chapters DRAFT:draft FIRST:first INDEX:index LAST:last NEXT:next PREV:prev PROFILE:profile REPOSITORY:repository STANDALONE:standalone TOP:top, echo "  $(lastword $(subst :, ,$(lstr))): $(LOCALIZATION_$(firstword $(subst :, ,$(lstr))))";) echo "..."; echo;
...
$(call unstyledeverything,$(LATEX),tex): $(FILEPREFIX)$(LATEX)/%.tex: $$(call srcs,$$*) $(YAML) $(srcdir)/template.tex $(srcdir)/pandoc-custom.py $(srcdir)/pandoc-latex.py
	$(makefolders)
	(cat $<; echo; echo; $(localizationyaml) echo "---"; cat $(YAML); $(fileyaml) echo "...") | pandoc -f markdown-smart -t latex-smart --standalone --template "$(srcdir)/template.tex" --filter "$(srcdir)/pandoc-custom.py" --filter "$(srcdir)/pandoc-latex.py" -o $@ --top-level-division=chapter $(if $(BIBLIOGRAPHY),--biblatex,)
	@echo "LaTeX file for $< generated at $@"

$(FILEPREFIX)$(LATEX)/$(INDEX).tex: $$(call unstyledeverything,$(LATEX),tex) $(YAML) $(srcdir)/pandoc-custom.py $(srcdir)/pandoc-latex.py
	$(makefolders)
	($(localizationyaml) echo "---"; cat $(YAML); echo "name: $(INDEX)"; echo "type: index"; echo "...") | pandoc -f markdown-smart -t latex-smart --standalone --template "$(srcdir)/template.tex" --filter "$(srcdir)/pandoc-custom.py" --filter "$(srcdir)/pandoc-latex.py" -o $@ --top-level-division=chapter $(if $(BIBLIOGRAPHY),--biblatex,)
...
$(patsubst $(FILEPREFIX)%,%,$(call allfiles,$(BUILD),cls,xxx,GO)): $(BUILD)/%/GO.xxx: $(YAML) $(srcdir)/template-index.tex $(srcdir)/pandoc-custom.py $(wildcard $(STYLES)/%.py)
	$(makefolders)
	($(localizationyaml) echo "---"; cat $(YAML); echo "style: $*"; echo "...") | pandoc -f markdown -t latex --standalone --template "$(srcdir)/template-index.tex" --filter "$(srcdir)/pandoc-custom.py"$(if $(wildcard $(STYLES)/$*.py), --filter $(STYLES)/$*.py,) -o $@
