...
$(call unstyledeverything,$(LATEX),tex): $(FILEPREFIX)$(LATEX)/%.tex: $$(call srcs,$$*) $(YAML) $(srcdir)/default.yml $(srcdir)/template.tex $(srcdir)/pandoc-custom.py $(srcdir)/pandoc-latex.py
	$(makefolders)
	(echo "---"; cat "$(srcdir)/default.yml"; echo "..."; echo "---"; cat $(YAML); echo "..."; echo "---"; $(fileyaml) echo 'outputfile: "$@"'; echo "..."; echo; cat $<) | $(PANDOC) -f markdown-smart -t latex-smart --standalone --template "$(srcdir)/template.tex" --filter "$(srcdir)/pandoc-custom.py" --filter "$(srcdir)/pandoc-latex.py" -o $@ --top-level-division=chapter $(if $(BIBLIOGRAPHY),--biblatex,)
...
$(FILEPREFIX)$(LATEX)/$(INDEX).tex: $$(call unstyledeverything,$(LATEX),tex) $(YAML) $(srcdir)/default.yml $(srcdir)/pandoc-custom.py $(srcdir)/pandoc-latex.py
	$(makefolders)
	(echo "---"; cat "$(srcdir)/default.yml"; echo "..."; echo "---"; cat $(YAML); echo "..."; echo "---"; echo "type: index"; echo 'outputfile: "$@"'; echo "...") | $(PANDOC) -f markdown-smart -t latex-smart --standalone --template "$(srcdir)/template.tex" --filter "$(srcdir)/pandoc-custom.py" --filter "$(srcdir)/pandoc-latex.py" -o $@ --top-level-division=chapter $(if $(BIBLIOGRAPHY),--biblatex,)
...
$(call allfiles,$(HTML),css,xhtml,$(basename $(BIBLIOGRAPHY))): $(FILEPREFIX)$(HTML)/%/$(basename $(BIBLIOGRAPHY)).xhtml: $(BIBLIOGRAPHY) $(YAML) $(srcdir)/default.yml $(STYLES)/%.css $(srcdir)/template.xhtml $(srcdir)/pandoc-custom.py $$(wildcard $(STYLES)/%.py) $(srcdir)/pandoc-html.py
	$(makefolders)
	(echo "---"; cat "$(srcdir)/default.yml"; echo "..."; echo "---"; echo "bibliography: $(addsuffix .bib,$(basename $(BIBLIOGRAPHY)))"; echo "citation-style: '$(realpath $(srcdir)/chicago-note-bibliography-16th-edition.csl)'"; echo "style: $(call styles,$@)"; echo "styles:"; echo "- name: $(call styles,$@)"; echo "  css: |"; echo '    ```{=html}'; cat $(STYLES)/$*.css | sed 's/^/    /'; echo '    ```'; $(if $(ALLSTYLES),$(foreach style,$(filter-out $*,$(stylenames)),echo "- name: $(style)"; echo "  css: |"; echo '    ```{=html}'; cat $(STYLES)/$(style).css | sed 's/^/    /'; echo '    ```';)) echo "..."; echo "---"; cat $(YAML); echo "..."; echo "---"; echo "type: bibliography"; echo "self: $(call relativepath,$(basename $(BIBLIOGRAPHY)).xhtml,$(basename $(BIBLIOGRAPHY))).xhtml"; echo "index: $(call relativepath,$(basename $(BIBLIOGRAPHY)).xhtml,$(INDEX)).xhtml"; echo "biblio: $(call relativepath,$(basename $(BIBLIOGRAPHY)).xhtml,$(basename $(BIBLIOGRAPHY))).xhtml"; echo "nocite: '@*'"; echo 'outputfile: "$@"'; echo "..."; echo '$(octothorpe) []{data-from-metadata="localization.type.biblio"}') | $(PANDOC) -f markdown-smart -t html5-smart --standalone --template "$(srcdir)/template.xhtml" --filter "$(srcdir)/pandoc-custom.py"$(if $(wildcard $(STYLES)/$*.py), --filter $(STYLES)/$*.py,) --filter "$(srcdir)/pandoc-html.py" --filter $(CITEPROC) -o $@ --self-contained --mathml
