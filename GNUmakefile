...
all everything: md xhtml tex pdf png$(if $(DEBUG), tex.json xhtml.json);
...
dist $(ZIP): zip;
clean: ; rm -rf $(BUILD)
unclean: ; rm -rf $(if $(DRAFTS),$(FILEPREFIX)$(MARKDOWN),) $(FILEPREFIX)$(LATEX) $(FILEPREFIX)$(LATEX).JSON $(FILEPREFIX)$(HTML) $(FILEPREFIX)$(HTML).JSON $(FILEPREFIX)$(PDF) $(FILEPREFIX)$(PNG) $(FILEPREFIX)$(ZIP)
clobber distclean gone: clean unclean;
zip: ;
.PHONY: all everything json htm html html5 xht xhtml5 xml $(HTML) $(HTML).JSON latex $(LATEX) $(LATEX).JSON markdown $(MARKDOWN) $(PDF) $(PNG) dist $(ZIP) clean unclean clobber distclean gone zip;
.SUFFIXES: ;
.SECONDEXPANSION: ;
...
makelatex = (echo "---"; cat "$(srcdir)/default.yml"; echo "..."; echo "---"; cat $(YAML); echo "..."; echo "---"; $(fileyaml) echo 'outputfile: "$@"'; echo "..."; echo; cat $<) | $(PANDOC) -f markdown-smart -t $(if $(1),json,latex-smart) --standalone --template "$(srcdir)/template.tex" --filter "$(srcdir)/pandoc-custom.py" --filter "$(srcdir)/pandoc-latex.py" -o $@ --top-level-division=chapter $(if $(BIBLIOGRAPHY),--biblatex,)
makelatexindexhead = (echo "---"; cat "$(srcdir)/default.yml"; echo "..."; echo "---"; cat $(YAML); echo "..."; echo "---"; echo "type: index"; echo 'outputfile: "$@"'; echo "...") | $(PANDOC) -f markdown-smart -t $(if $(1),json,latex-smart) --standalone --template "$(srcdir)/template.tex" --filter "$(srcdir)/pandoc-custom.py" --filter "$(srcdir)/pandoc-latex.py" -o $@ --top-level-division=chapter $(if $(BIBLIOGRAPHY),--biblatex,)

$(eval $(call unstyledtargets,$(LATEX),tex,$(INDEX)))
$(eval $(call unstyledtargets,$(LATEX).JSON,tex.json,$(INDEX)))

$(call unstyledeverything,$(LATEX),tex): $(FILEPREFIX)$(LATEX)/%.tex: $$(call srcs,$$*) $(YAML) $(srcdir)/default.yml $(srcdir)/template.tex $(srcdir)/pandoc-custom.py $(srcdir)/pandoc-latex.py$(if $(DEBUG), $(FILEPREFIX)$(LATEX).JSON/%.tex.json)
	$(makefolders)
	$(call makelatex)
...
$(FILEPREFIX)$(LATEX)/$(INDEX).tex: $$(call unstyledeverything,$(LATEX),tex) $(YAML) $(srcdir)/default.yml $(srcdir)/pandoc-custom.py $(srcdir)/pandoc-latex.py$(if $(DEBUG), $(FILEPREFIX)$(LATEX).JSON/$(INDEX).tex.json)
	$(makefolders)
...
$(call alleverything,$(HTML),css,xhtml): $(FILEPREFIX)$(HTML)/%.xhtml: $$(call srcs,$$(call filenames,$(HTML),$$(call styles,$$@),xhtml,$$@)) $(YAML) $(srcdir)/default.yml $(STYLES)/$$(call styles,$$@).css $(srcdir)/template.xhtml $(srcdir)/pandoc-custom.py $$(wildcard $(STYLES)/$$(call styles,$$@).py) $(srcdir)/pandoc-html.py$(if $(DEBUG), $(FILEPREFIX)$(HTML).JSON/%.xhtml.json)
	$(makefolders)
	$(call makehtml)
...
$(call allfiles,$(HTML),css,xhtml,$(INDEX)): $(FILEPREFIX)$(HTML)/%/$(INDEX).xhtml: $(allchaptersrcs) $(allappendixsrcs) $(allstandalonesrcs) $(YAML) $(srcdir)/default.yml $(STYLES)/%.css $(srcdir)/template.xhtml $(srcdir)/pandoc-custom.py $$(wildcard $(STYLES)/%.py) $(srcdir)/pandoc-html.py$(if $(DEBUG), $(FILEPREFIX)$(HTML).JSON/%/$(INDEX).xhtml.json)
	$(makefolders)
	$(call makehtmlindex)
...
$(call allfiles,$(HTML).JSON,css,xhtml.json,$(INDEX)): $(FILEPREFIX)$(HTML).JSON/%/$(INDEX).xhtml.json: $(allchaptersrcs) $(allappendixsrcs) $(allstandalonesrcs) $(YAML) $(srcdir)/default.yml $(STYLES)/%.css $(srcdir)/template.xhtml $(srcdir)/pandoc-custom.py $$(wildcard $(STYLES)/%.py) $(srcdir)/pandoc-html.py
	$(makefolders)
	$(call makehtmlindex,1)
	@echo "$* HTML JSON index generated at $@"

$(call allfiles,$(HTML),css,xhtml,$(basename $(BIBLIOGRAPHY))): $(FILEPREFIX)$(HTML)/%/$(basename $(BIBLIOGRAPHY)).xhtml: $(BIBLIOGRAPHY) $(YAML) $(srcdir)/default.yml $(STYLES)/%.css $(srcdir)/template.xhtml $(srcdir)/pandoc-custom.py $$(wildcard $(STYLES)/%.py) $(srcdir)/pandoc-html.py$(if $(DEBUG), $(FILEPREFIX)$(HTML).JSON/%/$(basename $(BIBLIOGRAPHY)).xhtml.json)
	$(makefolders)
	$(call makehtmlbibliography)
	@echo "$* HTML bibliography generated at $@"

$(call allfiles,$(HTML).JSON,css,xhtml.json,$(basename $(BIBLIOGRAPHY))): $(FILEPREFIX)$(HTML).JSON/%/$(basename $(BIBLIOGRAPHY)).xhtml: $(BIBLIOGRAPHY) $(YAML) $(srcdir)/default.yml $(STYLES)/%.css $(srcdir)/template.xhtml $(srcdir)/pandoc-custom.py $$(wildcard $(STYLES)/%.py) $(srcdir)/pandoc-html.py
	$(makefolders)
	$(call makehtmlbibliography)
